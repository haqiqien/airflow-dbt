...
def generate_vendors() -> pd.DataFrame:
    ...
    payment_terms_opts = ["Net 15", "Net 30",
                          "Net 45", "Net 60", "COD", "DP 50%"]
...
def generate_items() -> pd.DataFrame:
    ...
    adjectives = ["Premium", "Standar", "Economy",
                  "Pro", "Industrial", "Heavy Duty", "Lite"]
...
def generate_purchase_requests(dept_df: pd.DataFrame) -> pd.DataFrame:
    ...
    pr_statuses = ["Disetujui", "Disetujui",
...
def generate_po_lines(po_df: pd.DataFrame, items_df: pd.DataFrame) -> pd.DataFrame:
    active_items = items_df[items_df["is_active"]
...
def generate_goods_receipts(
    ...
    receivable_po_ids = set(
        po_df.loc[po_df["status"].isin(receivable_statuses), "po_id"])

    # Buat lookup po_date & expected_delivery dari po_df
    po_lookup = po_df.set_index(
        "po_id")[["po_date", "expected_delivery_date"]].copy()
    po_lookup["po_date"] = pd.to_datetime(po_lookup["po_date"])
    po_lookup["expected_delivery_date"] = pd.to_datetime(
        po_lookup["expected_delivery_date"])

    recv_lines = po_lines_df[po_lines_df["po_id"].isin(
        receivable_po_ids)].copy()
    ...
    inspection_opts = ["Lulus", "Lulus", "Lulus",
                       "Perlu Pemeriksaan Ulang", "Ditolak"]
    # hanya 3 opsi aktif, di-normalize di bawah
    inspection_weights = [70, 0, 0, 20, 10]

    scenario_opts = ["lengkap", "kurang_ringan",
                     "kurang_sedang", "kurang_parah", "lebih"]
    ...
    for _, line in recv_lines.iterrows():
        ...
        if on_time:
            lead_days = int(
                np.clip(np.random.lognormal(mean=2.5, sigma=0.5), 3, 30))
        ...
        if scenario == "lengkap":
            ...
            qty_received = max(
                1, round(qty_ordered * random.uniform(0.90, 0.99)))
        elif scenario == "kurang_sedang":
            qty_received = max(
                1, round(qty_ordered * random.uniform(0.75, 0.89)))
        elif scenario == "kurang_parah":
            qty_received = max(
                1, round(qty_ordered * random.uniform(0.50, 0.74)))
        else:  # lebih
...
def update_totals(
    ...
    po_df = po_df.drop(columns=["total_amount"]).merge(
        po_totals, on="po_id", how="left")
    ...
    pr_df = pr_df.drop(columns=["total_amount"]).merge(
        pr_totals, on="pr_id", how="left")
...
def export_csv(tables: dict[str, pd.DataFrame]) -> None:
    ...
    for name, df in tables.items():
        ...
        print(
            f"   ✓ {name}.csv  →  {len(df):>7,} baris | {len(df.columns):>2} kolom | {size_kb:>8.1f} KB")
...
def main() -> None:
    ...
    print(
        f"  Tabel transaksi utama     : purchase_order_lines = {len(po_lines_df):,} baris")
    ...
    print(
        f"  Min: {lt.min()}  |  Median: {lt.median():.0f}  |  Mean: {lt.mean():.1f}  |  Max: {lt.max()}")
    print(f"  On-time delivery rate: {gr_df['is_on_time'].mean() * 100:.1f}%")

    print("\n  ── Statistik Variance Qty (%) ──")
    vp = gr_df["variance_pct"]
    print(
        f"  Min: {vp.min():.1f}%  |  Mean: {vp.mean():.2f}%  |  Max: {vp.max():.1f}%")
    print("=" * 65)
    print("  Selesai! 🎉")
    print("=" * 65)
